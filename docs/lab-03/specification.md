...
- **Session** — `id`, `tokenHash` (unique, sha256 of the opaque token), `userId` (FK → User, cascade delete), `expiresAt`, `createdAt`. The token itself is never stored.
...
- **The migration writes one known password hash, deliberately.** Every account carried over from Lab 2 gets the bcrypt hash of `ChangeMe123!`, the password printed in the README. It is a choice with a real cost: on production data this would hand everyone a password an attacker could read in a public repository, and `mustChangePassword` would not stop them signing in and setting their own. It is taken anyway because §7.2's whole purpose is migrated Requesters who can actually sign in — without it, E2E-02 could only ever exercise a freshly seeded account, and the migrated half of AC-15 would be untestable by hand. A production version of this migration would write a distinct random value per row and require an Administrator reset; the SQL says so at the point where it writes the hash.
